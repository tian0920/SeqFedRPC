...
class MedMNIST(BaseDataset):
    def __init__(
        ...
        self.data = (
            torch.Tensor(np.load(root / "raw_data" / "xdata.npy")).float().unsqueeze(1)
        )
        self.targets = (
            torch.Tensor(np.load(root / "raw_data" / "ydata.npy")).long().squeeze()
        )
        self.general_data_transform = general_data_transform
        self.general_target_transform = general_target_transform
        self.train_data_transform = train_data_transform
...
class COVID19(BaseDataset):
    def __init__(
        ...
        if not isinstance(root, Path):
            root = Path(root)
        self.data = (
            torch.Tensor(np.load(root / "raw_data" / "xdata.npy"))
            .permute([0, -1, 1, 2])
            .float()
        )
        self.targets = (
...
class USPS(BaseDataset):
    def __init__(
        ...
        if not isinstance(root, Path):
            root = Path(root)
        train_part = torchvision.datasets.USPS(root / "raw_data", True, download=True)
        test_part = torchvision.datasets.USPS(root / "raw_data", False, download=True)
...
class SVHN(BaseDataset):
    def __init__(
        ...
        if not isinstance(root, Path):
            root = Path(root)
        train_part = torchvision.datasets.SVHN(root / "raw_data", "train", download=True)
        test_part = torchvision.datasets.SVHN(root / "raw_data", "test", download=True)
...
class TinyImagenet(BaseDataset):
    def __init__(
        ...
        if not os.path.isfile(root / "data.pt") or not os.path.isfile(
            ...
            data = []
            targets = []
            for cls in os.listdir(root / "raw_data" / "train"):
                for img_name in os.listdir(root / "raw_data" / "train" / cls / "images"):
                    img = pil_to_tensor(
                        Image.open(root / "raw_data" / "train" / cls / "images" / img_name)
                    ).float()
                    if img.shape[0] == 1:
            ...
            table = pd.read_table(
                root / "raw_data/val/val_annotations.txt",
                sep="\t",
                engine="python",
                header=None,
            )
            test_classes = dict(zip(table[0].tolist(), table[1].tolist()))
            for img_name in os.listdir(root / "raw_data" / "val" / "images"):
                img = pil_to_tensor(
                    Image.open(root / "raw_data" / "val" / "images" / img_name)
                ).float()
                if img.shape[0] == 1:
                    img = torch.expand_copy(img, [3, 64, 64])
                data.append(img)
    ...
    def __init__(
        ...
        if not os.path.isfile(root / "data.pt") or not os.path.isfile(
            ...
            for folder in ["test", "train", "valid"]:
                for cls in os.listdir(Path(root) / "raw_data" / folder):
                    for img_name in os.listdir(root / "raw_data" / folder / cls):
                        img = pil_to_tensor(
                            Image.open(root / "raw_data" / folder / cls / img_name)
                        ).float()
                        if img.shape[0] == 1:
